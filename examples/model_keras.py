...
def make_conv_layers_fc( model, dkconv=False, l2_reg=0.005, dropouts=[0.25,0.25,0.25,0.25,0.25] ):
    if dkconv:
        model.add(Convolution2D(24, (5, 5), activation='relu', padding='same', strides=(2,2), kernel_regularizer=l2_reg))
        ...
        model.add(Convolution2D(16, (8, 8), activation='relu', padding='same', strides=(4,4), kernel_regularizer=l2_reg))
        if dropouts[1] > 0.0:
            model.add(Dropout(dropouts[1]))
        model.add(Convolution2D(32, (4, 4), activation='relu', padding='same', strides=(2,2), kernel_regularizer=l2_reg))
        if dropouts[2] > 0.0:
            model.add(Dropout(dropouts[2]))
        model.add(Convolution2D(64, (3, 3), activation='relu', padding='same', strides=(2,2), kernel_regularizer=l2_reg))
    if dropouts[3] > 0.0:
...
def make_model_fc( num_actions, input_dim, dkconv=False, l2_reg=0.005, optimizer=None, dropouts=[0.25,0.25,0.25,0.25,0.25], metrics=None ):
    model = Sequential()
    if K.image_dim_ordering() == 'tf':
        # (width, height, channels)
        model.add(Permute((2, 3, 1), input_shape=input_dim))
    elif K.image_dim_ordering() == 'th':
        # (channels, width, height)
        model.add(Permute((1, 2, 3), input_shape=input_dim))
    else:
